...
-- Note: we are filtering for only 1 month on this View to keep the performance under control
--       since all downstream usages of this need a max of 7 days
...
SELECT
    ...
    inserted_timestamp,
    s3_file_path
FROM {{ source('party_planner_realtime', 'ad_show_effective') }}
WHERE date_utc >= {{ var('ds') }}::DATE - INTERVAL '31 days'
...
UNION ALL SELECT
    ...
    "payload.ad_unit_id" AS "payload.ad_event_data.ad_unit_id",
    "payload.advertisement.creative_id" AS "payload.ad_event_data.advertisement.creative_id",
    "payload.advertisement.format" AS "payload.ad_event_data.advertisement.format",
    "payload.advertisement.size.height" AS "payload.ad_event_data.advertisement.size.height",
    "payload.advertisement.size.width" AS "payload.ad_event_data.advertisement.size.width",
    ...
    "payload.network" AS "payload.ad_event_data.network",
    "payload.placement" AS "payload.ad_event_data.placement",
    NULL AS "payload.ad_event_data.ppid",
    NULL AS "payload.ad_event_data.request_id",
    "payload.request_type" AS "payload.ad_event_data.request_type",
    "payload.screen" AS "payload.ad_event_data.screen",
    ...
    inserted_timestamp,
    s3_file_path
FROM {{ source('party_planner_realtime', 'impression') }}
WHERE date_utc >= {{ var('ds') }}::DATE - INTERVAL '31 days'
