...
WITH adjust_export_deduped AS (
    SELECT
        {{ dbt_utils.generate_surrogate_key(['adjust_id', 'client_type']) }} AS surrogate_key,
        client_type,
        date_utc AS event_received_date_utc,  -- this is just the export date, not necessarily same as event timestamp
        activity_kind,
        app_id,
        app_name,
        adjust_id,
    ...
    WHERE (activity_kind IN ('install', 'install_update'))
    QUALIFY ROW_NUMBER() OVER (PARTITION BY app_name, adjust_id ORDER BY decode(activity_kind, 'install_update', 1, 'install', 2)) = 1
),

adjust_callbacks_deduped AS (
    SELECT
        {{ dbt_utils.generate_surrogate_key(['adid', 'client_type']) }} AS surrogate_key,
        adid AS adjust_id,
    ...
    {% if is_incremental() %}
        ...
        (installed_at >= {{ var('ds') }}::DATE - INTERVAL '120 DAYS')
        AND (NVL(environment, 'production') = 'production')
        AND (surrogate_key NOT IN (SELECT surrogate_key FROM {{ this }}))
    {% else %}
        -- doing full load from this raw data table is probably not realistic
        (installed_at >= {{ var('ds') }}::DATE - INTERVAL '120 DAYS')
        AND (NVL(environment, 'production') = 'production')
    {% endif %}

    QUALIFY ROW_NUMBER() OVER (PARTITION BY app_name, adjust_id ORDER BY installed_at) = 1
)

SELECT
    surrogate_key,
    a.event_received_date_utc AS event_received_date_utc,
    COALESCE(a.adjust_id, b.adjust_id)::VARCHAR(32) AS adjust_id,
    ...
    COALESCE(a.is_untrusted, b.is_untrusted, FALSE) AS is_untrusted,
