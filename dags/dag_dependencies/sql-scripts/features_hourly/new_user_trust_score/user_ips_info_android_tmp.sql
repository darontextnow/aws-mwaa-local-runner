CREATE TRANSIENT TABLE user_ips_info_android_tmp_{{ ts_nodash }} AS
    ...
    WITH tmp_missing_geo_ips AS (
        WITH unk_ip_addresses AS (
            ...
            WHERE
                ...
                AND (client_type IN ('TN_ANDROID', '2L_ANDROID', 'TN_WEB', 'any')) --any includes all msg_ips
        ),
...
latest_asn AS (
    ...
    FROM (
        ...
        ips_asn_match_first_1_octets AS (
            ...
            JOIN analytics.maxmindasn_geolite2_asn_blocks_ipv4 b
            ON (SPLIT_PART(network, '.', 1) = SPLIT_PART(ip_address, '.', 1))
        )
...
WHERE (client_type IN ('TN_ANDROID', '2L_ANDROID', 'TN_WEB', 'any')) --any includes all msg_ips
